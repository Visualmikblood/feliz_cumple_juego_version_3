-- Esquema de base de datos para el juego de cumpleaños multijugador
...
-- Base de datos (crear manualmente en localhost/hosting)
-- CREATE DATABASE IF NOT EXISTS birthday_game;
-- USE birthday_game;
...
-- Tabla de mensajes de felicitaciones (estáticos del juego)
...
INSERT INTO `congratulation_messages` (`friend_id`, `name`, `message`, `color_class`, `icon_name`, `photo_url`) VALUES
...
CREATE INDEX idx_rooms_status_expires ON `game_rooms` (`status`, `expires_at`);
CREATE INDEX idx_players_room_finished ON `players` (`room_id`, `has_finished_rating`);
CREATE INDEX idx_ratings_room_message ON `ratings` (`room_id`, `message_id`);
CREATE INDEX idx_notifications_room_type ON `notifications` (`room_id`, `type`, `is_sent`);
